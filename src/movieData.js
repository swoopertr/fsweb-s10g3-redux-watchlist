export async function getMovies () {
    const options = {
      ...
      try {
        let response = await fetch('https://api.themoviedb.org/3/movie/popular', options);
        const result = await response.json();
        const movies = movieOrganizer(result.results)
        return movies;
      } catch (error) {
        return error
      }
...
const movieOrganizer = (movieArray) => {
    const pre_poster_path = 'https://image.tmdb.org/t/p/w500';
    ...
    for (let i = 0; i < movieArray.length; i++) {
        ...
        optimizedArray.push(tmpMovieObj);
    }
